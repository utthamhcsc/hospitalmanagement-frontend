   ...
   const [data,setData]=React.useState([]);
    const formik=useFormik({
       ...
       onSubmit:values=>{
        ...
        PostFormdata('financeincome/add','POST',{...values,date:new Date(values.date)=='Invalid Date'?'':new Date(values.date)}).then(
            data=>{
                console.log(data)
                if(values.id){
         ...
         validationSchema:Yup.object().shape({
           ...
           // incomeHeadId:Yup.date().required('Required'),
...
React.useEffect(()=>{
    ...
    Getdata('incomeHead/get').then(data=>{setData(data);console.log(data)})
            ...
            <button type="button" class="close " data-dismiss="modal" aria-label="Close">
                ...
                <form onSubmit={formik.handleSubmit}>
                    ...
                    <div className="form-row">
                        <div className="col-6">
                    <div>Income Head<small className="req text-danger"> *</small>
                    <select id="input" className="form-control" value={formik.values.incomeHeadId} onChange={(e)=>formik.setFieldValue('incomeHeadId',e.target.value)} >
                        <option>Select</option>
                      {
                          (data||[]).map(item=><option value={item.id}>{item.incomeHead}</option>)
                      }
                    </select>
                    <span className='text-danger'>{(formik.touched.incomeHeadId && formik.errors.incomeHeadId)?formik.errors.incomeHeadId:''}</span>
                    </div>
                    </div>
