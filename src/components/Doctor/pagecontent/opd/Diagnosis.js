...
export default function Diagnosis(props) 
{
  ...
  data-toggle='modal' data-target='#addDiagnosis'>
    ...
    <button onClick={()=>Getdata(`myopddiagnosis/delete/${rowData.opdId}`).then(data=>setdataSrc(item=>item.filter(item=>item.id!=data.id)))} className={'btn btn-xs btn-light'} ><i className='fa fa-trash'></i></button>
    ...
    <nav aria-label="breadcrumb" >
      <ol class="p-2 px-5" style={{backgroundColor:'#3f51b5'}} >
        <li class="text-white font-weight-bold d-sm-flex justify-content-between align-items-baseline text-xs " aria-current="page">
         ...
         <NavLink to={`/doctor/myopd/charges/${patientId}`} 
    ...
    class="btn btn-light text-xs  btn-xs  ml-1"><i className='fa fa-reorder'/>  Bill</NavLink>  
      ...
      document: "",
      description: "",
      opdId: ""})} data-target="#addDiagnosis"  class="btn btn-light text-xs  btn-xs  ml-1"> <i class="fa fa-plus"></i> Add Diagnosis</button>               
          </div>
        </li>
      </ol>
    </nav>
