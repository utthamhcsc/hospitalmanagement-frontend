...
export default function CallLog(props) 
{
    ...
    const columnDefs=[{targets:-1,orderable:false,responsivePriority:1,createdCell:(td,cellData,rowData,row,col)=>ReactDOM.render(
      ...
      <button onClick={()=>props.setindex({})} className={'btn btn-xs btn-success'} data-toggle='modal' data-target='#calllog'><i className='fa fa-pencil'></i></button>
      ...
      <ol class="p-2 px-5" style={{backgroundColor:'#3f51b5'}} >
        <li class="text-white font-weight-bold d-sm-flex justify-content-between align-items-baseline" aria-current="page">
          ...
          <div className='btn-group p-0'>
            ...
            <button data-toggle="modal" onClick={()=>props.setindex({})} data-target='#calllog' class="btn btn-light text-xs  btn-xs  ml-1"> <i class="fa fa-plus"></i> Add CallLog</button>               
          </div>
        </li>
      </ol>
    </nav>
