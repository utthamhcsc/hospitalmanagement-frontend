...
export default function PostalReceive(props) {
    ...
    const columnDefs=[{targets:-1,orderable:false,responsivePriority:1,createdCell:(td,cellData,rowData,row,col)=>ReactDOM.render(
      ...
      <button onClick={()=>props.setindex(rowData)} className={'btn btn-xs btn-success'} data-toggle='modal' data-target='#PostalRcv'><i className='fa fa-pencil'></i></button>
  ...
  <div className='btn-group p-0'>
    ...
    <button data-toggle="modal" onClick={()=>props.setindex({})} data-target="#PostalRcv" class="btn btn-light text-xs  btn-xs  ml-1"> <i class="fa fa-plus"></i> Add PostalReceive</button>               
  </div>
  </li>
  </ol>
  </nav>
