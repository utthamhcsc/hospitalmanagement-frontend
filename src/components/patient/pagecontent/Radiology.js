...
export default function Radiology(props)
 {
  ...
  const [index1,setindex1]=React.useState({});
    ...
    const columnDefs=[{targets:-1,orderable:false,responsivePriority:1,createdCell:(td,cellData,rowData,row,col)=>ReactDOM.render(
      ...
      <button onClick={()=>setindex(rowData.p)} className={'btn btn-xs btn-success'} data-toggle='modal' data-target='#addPatient'><i className='fa fa-pencil'></i></button>
      
      <button onClick={()=>Getdata(`radiologypatient/delete/${rowData.p.id}`).then(data=>setdataSrc(item=>item.filter(item1=>item1.p.id!=data)))} className={'btn btn-xs btn-danger'} ><i className='fa fa-trash'></i></button>
      
      </BrowserRouter>,td)}]
    const Link=<NavLink to='dfgh'/>

    React.useEffect(()=>{
        Getdata('radiologypatient/get/patient/'+window.localStorage.getItem('userId')).then(data=>setdataSrc(data));
      },[])
