...
export default function Radiology(props)
 {
  ...
  const [index1,setindex1]=React.useState({});
    ...
    const columnDefs=[{targets:-1,orderable:false,responsivePriority:1,createdCell:(td,cellData,rowData,row,col)=>ReactDOM.render(
      ...
      <button onClick={()=>setindex(rowData.p)} className={'btn btn-xs btn-warning'} data-toggle='modal' data-target='#AddradiologyTest'><i className='fa fa-pencil'></i></button>
      
      <button onClick={()=>deletealert(`radiology/delete`,`${rowData.p.id}`)} className={'btn btn-xs btn-danger'} ><i className='fa fa-trash'></i></button>
...
<div className='btn-group '>
    ...
    <button data-toggle="modal" onClick={()=>setindex('')} data-target="#AddradiologyTest" class="btn btn-primary text-xs  btn-xs  ml-1"> <i class="fa fa-plus"></i> Add Test</button>               
    <NavLink to='/admin/radiology/testreport' className='btn btn-xs btn-primary mx-1'>
