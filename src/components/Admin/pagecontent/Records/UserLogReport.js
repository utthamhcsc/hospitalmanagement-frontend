...
export default ()=> {
  ...
  const formik=useFormik({
      ...
      onSubmit:v=>{
          console.log(v)
          if(v.type=='all'){
            Getdata('userlog/get').then(data=>{setdataSrc(data);
                console.log(data)
                });
          }
          else{
            PostFormdata('userlog/getbyDate','POST',{fromDate:v.fromDate.toJSON(),toDate:v.toDate.toJSON()}).then(data=>{setdataSrc(data);
                console.log(data)
                });
          }
      }
...
const column=[{data:'user',title:'Name'},
    {data:'loginDatetime',title:'Login Date Time',render:(data,type,row,meta)=>new Date(data)=='Invalid Date'?'':new Date(data)},
