...
export default function PharmacyBillReport() {
  ...
  const formik=useFormik({
      ...
      onSubmit:v=>{
          console.log(v)
          if(v.type=='all'){
            Getdata('pharmacyBillBasic/get').then(data=>{setdataSrc(data);
                console.log(data)
                });
          }
          else{
            PostFormdata('pharmacyBillBasic/getbyDate','POST',{fromDate:v.fromDate.toJSON(),toDate:v.toDate.toJSON()}).then(data=>{setdataSrc(data);
                console.log(data)
                });
          }
      }
