...
export default function IncomeHead(props)
 {
  ...
  {data:'action',title:'Action'}]
    ...
    React.useEffect(()=>{
        Getdata('incomeHead/get').then(data=>setdataSrc(data));
      },[])
  ...
  <li class="text-white font-weight-bold d-sm-flex justify-content-between align-items-baseline" aria-current="page">
      <h6 className='text-sm' style={{letterSpacing:'1px',lineHeight:'100%'}}>Income Head List</h6>
  <div className='btn-group p-0'>
    <button className={'btn btn-xs  btn-light ml-1 ' } style={{marginLeft:'0.5px !important',opacity:0}} data-toggle="modal" data-target="sdf">dfgh</button>
    <button data-toggle="modal" onClick={()=>setindex1({incomeHead:'',description:''})} data-target="#incomeHead" class="btn btn-light text-xs  btn-xs  ml-1"> <i class="fa fa-plus"></i> Add Income Head</button>               
                         </div>
  </li>
  </ol>
  </nav>
  ...
  <div className='px-5 pb-5'>
    ...
    <AddIncomeHead data={index1} setdataSrc={setdataSrc}/>
