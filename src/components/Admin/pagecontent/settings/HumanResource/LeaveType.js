...
import AddLeaveType from '../../../../../Forms/Settings/humanresource/AddLeaveType'
...
export default function BedType(props)
 {
  ...
  {data:'action',title:'Action'}]
    ...
    const columnDefs=[{targets:-1,orderable:false,responsivePriority:1,createdCell:(td,cellData,rowData,row,col)=>ReactDOM.render(
      ...
      data-toggle='modal' data-target='#leaveType'><i className='fa fa-pencil'></i></button>
      
      <button onClick={()=>deletealert(`leaveType/delete`,`${rowData.id}`)
      } 
    ...
    React.useEffect(()=>{
        Getdata('leaveType/get').then(data=>setdataSrc(data));
      },[])
  ...
  <li class="text-white font-weight-bold d-sm-flex justify-content-between align-items-baseline" aria-current="page">
      <h6 className='text-sm' style={{letterSpacing:'1px',lineHeight:'100%'}}>Leave Type</h6>
  <div className='btn-group p-0'>
    <button className={'btn btn-xs  btn-light ml-1 ' } style={{marginLeft:'0.5px !important',opacity:0}} data-toggle="modal" data-target="sdf">dfgh</button>
    <button data-toggle="modal" onClick={()=>setindex1({name:''})} 
    data-target="#leaveType" class="btn btn-light text-xs  btn-xs  ml-1"> 
    <i class="fa fa-plus"></i> Add Leave Type</button>               
                         </div>
  </li>
  </ol>
  </nav>
  ...
  <div className='px-5 pb-5'>
    ...
    <AddLeaveType data={index1} setdataSrc={setdataSrc}/>
