...
let { patientId } = useParams();
   ...
   {targets:0,orderable:false,responsivePriority:1,createdCell:(td,cellData,rowData,row,col)=>ReactDOM.render(
    <BrowserRouter>
    <a onClick={()=>history.push(`/accountant/ipd1/consultentRegister/${rowData.patientId}`)} href='javascript:void(0)'>{cellData}</a>
    </BrowserRouter>,td)}
    ...
    <NavLink to={ '/accountant/ipd1/consultentRegister/'+patientId} activeClassName='active' class="btn btn-light text-xs  btn-xs ml-1 ">
     <i class="fa fa-reorder"></i> Consultant Register
    </NavLink>             
    <NavLink to={ '/accountant/ipd1/diagnosis/'+patientId} activeClassName='active' class="btn btn-light text-xs  btn-xs ml-1 ">
     <i class="fa fa-reorder"></i> Diagnosis
    </NavLink>             
    <NavLink to={ '/accountant/ipd1/ipdPresciption/'+patientId} activeClassName='active' class="btn btn-light text-xs  btn-xs ml-1 ">
