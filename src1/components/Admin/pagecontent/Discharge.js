...
export default function Discharge(props) {
 ...
 {targets:0,orderable:false,responsivePriority:1,createdCell:(td,cellData,rowData,row,col)=>ReactDOM.render(
  <BrowserRouter>
  <a onClick={()=>history.push(`/admin/ipd1/consultentRegister/${rowData.ipdId}`)} href='javascript:void(0)'>{cellData}</a>
  </BrowserRouter>,td)}
 ...
 const Link=<NavLink to='dfgh'/>
    ...
    React.useEffect(()=>{Getdata('inpatient/admin/'+props.adminId+'/YES').then(data=>setdataSrc(data));},[])
    ...
    return (
        <>
          ...
          <div className='px-5 pb-5'>
            <Table id='discharge' col={column} dataSrc={dataSrc} columnDefs={columnDefs}/>
          </div>
