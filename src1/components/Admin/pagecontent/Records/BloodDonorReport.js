...
export default ()=> {
  ...
  const formik=useFormik({
      ...
      onSubmit:v=>{
          console.log(v)
          if(v.type=='all'){
            Getdata('donateblood/get').then(data=>{setdataSrc(data);
                console.log(data)
                });
          }
          else{
            PostFormdata('donateblood/getbyDate','POST',{fromDate:v.fromDate.toJSON(),toDate:v.toDate.toJSON()}).then(data=>{setdataSrc(data);
                console.log(data)
                });
          }
      }
...
const column=[ {data:'d.donateDate',title:'Date',
render:(data,type,row,meta)=>new Date(data)=='Invalid Date'?'':new Date(data).toLocaleDateString()
},{data:'b.donarName',title:'Donar Name'},
    {data:'b.ageyear',title:'Age'},
    {data:'b.bloodGroup',title:'Blood Group'},
    {data:'b.gender',title:'Gender'},
    {data:'b.contact',title:'Contact'},
    {data:'d.bagNo',title:'Bag No'},
   {data:'d.lot',title:'Lot'},
   {data:'d.quantity',title:'Quantity(in Ml)'},
