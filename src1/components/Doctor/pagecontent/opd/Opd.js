...
import AddPat from '../../../../Forms/OPDForms/AddDoctorPatient'
...
export default function Opd(props)
{
    ...
    <BrowserRouter>
    <button className={'btn btn-link'} onClick={()=>history.push(`/doctor/myopd/visits/${rowData.patientId}`)} >{cellData}</button>
    </BrowserRouter>,td)}
  ]
    const link=<NavLink to='dfgh'/>
    React.useEffect(()=>{
      Getdata('myopd/get/doctor/'+window.localStorage.getItem('userId')).then(data=>{console.log(data);setdataSrc(data)});},[])
