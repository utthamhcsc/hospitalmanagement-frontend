...
import {NavLink,BrowserRouter,Link,useHistory, withRouter} from 'react-router-dom'
...
export default withRouter((props)=>
{
    ...
    <BrowserRouter>
    <button className={'btn btn-link'} onClick={()=>history.push(`/patient/myopd/visits/${rowData.patientId}`)} >{cellData}</button>
    </BrowserRouter>,td)}
  ]
    const link=<NavLink to='dfgh'/>
    React.useEffect(()=>{
      history.push('/patient/myopd/visits/'+window.localStorage.getItem('userId'))
    },[])
     // Getdata('myopd/get/patient/'+window.localStorage.getItem('userId')).then(data=>{console.log(data);setdataSrc(data)});},[])
