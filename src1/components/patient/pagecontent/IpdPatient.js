...
let { patientId } = useParams();
   ...
   {targets:0,orderable:false,responsivePriority:1,createdCell:(td,cellData,rowData,row,col)=>ReactDOM.render(
    <BrowserRouter>
    <a onClick={()=>history.push(`/patient/ipd1/consultentRegister/${rowData.patientId}`)} href='javascript:void(0)'>{cellData}</a>
    </BrowserRouter>,td)}
   
  
  ]
  
    React.useEffect(()=>{Getdata('inpatient/patient/'+props.patientId+'/NO').then(data=>setdataSrc(data));},[])
    ...
    <NavLink to={ '/patient/ipd1/diagnosis/'+patientId} activeClassName='active' class="btn btn-light text-xs  btn-xs ml-1 ">
    ...
    <IPDAddpat data={index} patientId={props.patientId}/>
    </div>
   </>
  )
}
